!(function () {
  var e,
    ...
    a = '<div class="well-path-widget-link-row">{links}</div>',
    d =
      '<div class="well-path-widget-link-wrap"><span data-href="{href}">{label}</span></div>',
    w = function () {
      e.remove(),
    ...
    o = function (e) {
      "span" == e.target.nodeName.toLowerCase() &&
        -1 !=
        ...
        w()),
        "img" == e.target.nodeName.toLowerCase() &&
          -1 !=
            e.target.parentElement.className.indexOf(
  ...
  -1 == window.location.href.indexOf("securly.com") &&
    (function () {
      chrome.runtime.onMessage.addListener((p, s) => {
        if (
          ...
          "object" == typeof p &&
            void 0 !== p.action &&
            void 0 !== p.source &&
            "well-path-widget" == p.source)
        )
          switch (p.action) {
            case "display":
              if (void 0 === p.data || t) return;
              !(function (w) {
                ...
                for (i = 0; i < w.links.length; i++) {
                  var s = w.links[i],
                    r = void 0 !== s.email ? "mailto:" + s.email : s.url;
                  (p += d.replace("{href}", r).replace("{label}", s.label)),
                    i % 2 == 1 &&
                      ((widgetLinks += a.replace("{links}", p)), (p = ""));
                }
                p && (widgetLinks += a.replace("{links}", p)),
                  (widgetHtml = l
                    .replace(
                      "{close_btn_src}",
                      chrome.extension.getURL("icon-16-px-close.svg"),
                    )
                    .replace("{header}", w.header)
                    .replace("{body}", w.body)
      ...
      (p.rel = "stylesheet"),
        (p.href = chrome.extension.getURL("fonts/Metropolis.css")),
